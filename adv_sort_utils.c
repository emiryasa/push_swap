...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/27 14:52:21 by eyasa             #+#    #+#             */
/*   Updated: 2024/06/27 14:52:22 by eyasa            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int largest_smaller_number(t_list *a_node, t_list *stack_b)
{
    ...
    if (*(int *)a_node->content > *(int *)value->content)
    {
        while (b_node)
        {
            if (*(int *)b_node->content < *(int *)a_node->content && *(int *)value->content < *(int *)b_node->content)
                value = b_node;
            b_node = b_node->next;
        }
    }
...
int smallest_large_number(t_list *b_node, t_list *stack_a)
{
    ...
    if (*(int *)b_node->content < *(int *)value->content)
    {
        while (a_node)
        {
            if (*(int *)a_node->content > *(int *)b_node->content && *(int *)value->content > *(int *)a_node->content)
                value = a_node;
            a_node = a_node->next;
        }
    }
